...
def test_sync_write_does_not_retry_transport_error(force_no_refresh, method):
    """Sync writes propagate a dropped connection unless the caller opts in."""
    handler, calls = _transport_seq_handler([httpx.ReadError("boom")])
    client = _make_fixed_client(handler)
    try:
        with pytest.raises(httpx.ReadError):
            client._request_with_refresh_sync(method, "/api/things")
        assert len(calls) == 1
    finally:
        client._sync_http.close()
...
@pytest.mark.asyncio
@pytest.mark.parametrize("method", ["post", "patch", "put", "delete"])
async def test_async_write_does_not_retry_transport_error(force_no_refresh, method):
    """Writes propagate a dropped connection without an explicit opt-in."""
    handler, calls = _transport_seq_handler([httpx.ReadError("boom")])
    client = _make_fixed_client(handler)
    try:
        with pytest.raises(httpx.ReadError):
            await client.request(method, "/api/things")
        assert len(calls) == 1
    finally:
        await client.close()
