...
def test_batch_replacement_invalidates_conditional_update_precondition(
    e2e_client, platform_admin,
):
    table_id = _create_policy_table(e2e_client, platform_admin)
    headers = platform_admin.headers
    path = f"/api/tables/{table_id}/documents"
    try:
        created = e2e_client.post(
            path, headers=headers, json={"data": {"state": "reviewed"}},
        )
        assert created.status_code == 201, created.text
        original = created.json()
        replaced = e2e_client.post(
            path + "/batch", headers=headers,
            json=_replace_body((original["id"], {"state": "replaced"})),
        )
        assert replaced.status_code == 200, replaced.text
        stale = e2e_client.patch(
            path + f"/{original['id']}/conditional", headers=headers,
            json={
                "data": {"state": "stale-write"},
                "expected_updated_at": original["updated_at"],
            },
        )
        assert stale.status_code == 409, stale.text
        current = e2e_client.get(path + f"/{original['id']}", headers=headers)
        assert current.status_code == 200, current.text
        assert current.json()["data"] == {"state": "replaced"}
    finally:
