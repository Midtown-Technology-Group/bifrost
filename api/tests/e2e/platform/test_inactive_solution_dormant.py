...
def _deploy_with_workflow_app_and_table(e2e_client, headers, sid: str) -> tuple[str, str, str]:
    ...
    return app_id, table_name, table_id
...
async def test_solution_dormant_gate_for_workflow_app_and_browser(
    ...
    app_id, table_name, table_id = _deploy_with_workflow_app_and_table(e2e_client, headers, sid)
    workflow_body = {
    ...
    # The direct UUID route must not bypass an inactive install's app gate.
    direct_write = e2e_client.post(
        f"/api/tables/{table_id}/documents",
        headers=headers,
        json={"id": "direct", "data": {"val": "x"}},
    )
    assert direct_write.status_code == 404, direct_write.text
    direct_read = e2e_client.get(
        f"/api/tables/{table_id}/documents/probe", headers=headers
    )
    assert direct_read.status_code == 404, direct_read.text
