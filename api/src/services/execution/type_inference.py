...
from typing import TYPE_CHECKING, Annotated, Any, Literal, Union, get_args, get_origin, get_type_hints
...
def get_ui_type(python_type: Any) -> str:
    ...
    if get_origin(python_type) is Annotated:
        return get_ui_type(get_args(python_type)[0])

    # Direct mapping
    if isinstance(python_type, type) and python_type in TYPE_MAPPING:
...
def _literal_schema_from_values(values: list[Any]) -> dict[str, Any]:
    # An enum already describes mixed JSON types without narrowing to its first member.
    if not values:
        return {}
    schema: dict[str, Any] = {"enum": values}
    types = {_JSON_TYPE_MAPPING.get(type(value), "null" if value is None else None) for value in values}
...
def build_json_schema(python_type: Any) -> dict[str, Any]:
    """Infer validation without narrowing null unions or unknown annotations."""
    if python_type is inspect.Parameter.empty or python_type is Any or python_type is object:
        return {}
    if python_type is type(None):
        return {"type": "null"}
    if isinstance(python_type, type) and issubclass(python_type, Enum):
        return _literal_schema_from_values([member.value for member in python_type])
    if isinstance(python_type, type) and python_type in _JSON_TYPE_MAPPING:
    ...
    if origin is Annotated:
        return build_json_schema(args[0]) if args else {}
    if origin in {list, Sequence} or (python_type is list or python_type is Sequence):
        return {"type": "array", "items": build_json_schema(args[0]) if args else {}}
    if origin in {dict, Mapping} or (python_type is dict or python_type is Mapping):
        return {"type": "object", "additionalProperties": (build_json_schema(args[1]) or True) if len(args) >= 2 else True}
    if origin is tuple:
        if len(args) == 2 and args[1] is Ellipsis:
            return {"type": "array", "items": build_json_schema(args[0])}
        return {"type": "array", "prefixItems": [build_json_schema(arg) for arg in args], "minItems": len(args), "maxItems": len(args)}
    if origin is Union or type(python_type).__name__ == "UnionType":
        return {"anyOf": [build_json_schema(arg) for arg in args]}
    return {}
...
def extract_parameters_from_signature(func: Any) -> list[dict[str, Any]]:
    ...
    Note:
        ...
        - Parameters without type hints keep the legacy string UI but accept any JSON value
    ...
    try:
        ...
        for param_name, param in sig.parameters.items():
            ...
            if has_default and default_value is not None:
                ...
                if isinstance(default_value, (str, int, float, bool, list, dict, type(None))):
