...
On 2026-09-24/25 UTC, a read-only Azure Monitor query over the preceding 24 hours returned 10,631 requests, 230 HTTP 5xx responses, a mean of hourly mean working-set samples of 1.70 GB, and a maximum working-set sample of 3.00 GB for `app-mtg-bifrost-production`. A later live readback found two B3 plan instances; instance count throughout this historical window was not established. Live settings contain `BIFROST_MAX_CONCURRENCY=4`, `BIFROST_MAX_WORKERS=4`, `BIFROST_DATABASE_POOL_SIZE=2`, and `BIFROST_DATABASE_MAX_OVERFLOW=3`.
...
The production 24-hour execution snapshot recorded 3,069 successful executions over about 24.11 hours. Historical plan instance-hours were not verified, so this snapshot does **not** establish successful executions per B3-hour. Other app roles and workload types also shared the plan. The clean synthetic HTTP fixture peaked at 18.57 executions/second, equivalent to 66,852 such fixture executions/hour on a four-core VM. Its short calls, local dependencies, and missing client/renderer make that figure unsuitable for a production cost forecast. A language change cannot be justified from either number without a representative mix and measured memory per role at the same useful-work rate.
