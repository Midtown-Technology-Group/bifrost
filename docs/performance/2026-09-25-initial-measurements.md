...
The production 24-hour execution snapshot recorded 3,069 successful executions over about 24.11 hours, or **127 successful executions per observed B3-hour**. This is useful work divided by running plan time, not B3 capacity: other app roles and workload types shared the plan. The clean synthetic HTTP fixture peaked at 18.57 executions/second, equivalent to 66,852 such fixture executions/hour on a four-core VM. Its short calls, local dependencies, and missing client/renderer make that figure unsuitable for a production cost forecast. A language change cannot be justified from either number without a representative mix and measured memory per role at the same useful-work rate.
