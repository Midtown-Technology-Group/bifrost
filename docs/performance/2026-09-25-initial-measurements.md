...
On 2026-09-24/25 UTC, a read-only Azure Monitor query over the preceding 24 hours returned 10,631 requests, 230 HTTP 5xx responses, a mean of hourly mean working-set samples of 1.70 GB, and a maximum working-set sample of 3.00 GB for `app-mtg-bifrost-production`. A later live readback found two B3 plan instances; [Azure change history](2026-09-25-production-compute-denominator.md) verified one instance throughout the earlier execution sample, which ended before the scale-up. Live settings contain `BIFROST_MAX_CONCURRENCY=4`, `BIFROST_MAX_WORKERS=4`, `BIFROST_DATABASE_POOL_SIZE=2`, and `BIFROST_DATABASE_MAX_OVERFLOW=3`.
...
### Memory and useful work per compute-hour

The clean `12f7cc538` outbound HTTP sweep's cgroup samples peaked at 353 MiB for the API, 366 MiB for the worker, and 426 MiB for the scheduler; these are independent role maxima, not one simultaneous sum. The fixed-agent run's worker reached 422 MiB, though another test stack was active. PostgreSQL, Redis, RabbitMQ, and PgBouncer were separate lab containers. The lab lacks production client and renderer containers, and its CPU and memory hardware differ from Azure B3. Production App Service working set reached about 3 GB at an app-wide sample, with no per-container attribution. A read-only `GET /api/platform/workers/metrics?range=24h` returned only four worker memory points across three worker IDs, too sparse to estimate sustained worker density; the current pool response reported memory for only one of three pools. Memory is therefore a live decision gate, but neither a Go density advantage nor a safe higher worker-slot count is established.

The production execution snapshot recorded 3,069 successful executions from the requested start through its last included record, about 24.11 hours. [Plan change history](2026-09-25-production-compute-denominator.md) verifies one B3 instance across that window, giving approximately 127 **observed** successful executions per B3-hour. The query completion time was not recorded, so 127 is an approximate upper bound on the observed rate. Other app roles and workload types shared the plan; this is not capacity. The clean synthetic HTTP fixture peaked at 18.57 executions/second, equivalent to 66,852 such fixture executions/hour on a four-core VM. Its short calls, local dependencies, and missing client/renderer make that figure unsuitable for a production cost forecast. A language change cannot be justified from either number without a representative mix and measured memory per role at the same useful-work rate.
