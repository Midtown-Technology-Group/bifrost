...
function getSubscriptionMeta(subscription: EventSubscription) {
	...
	const resourceName = isAgent
		...
		: subscription.workflow_name ||
			...
			"Unknown Workflow";
	const executedCount = subscription.success_count + subscription.failed_count;
	const successRate =
		executedCount > 0
			? Math.round(
					(subscription.success_count / executedCount) *
						100,
				)
			: null;
...
function SubscriptionEventType({
 ...
 criteria: EventSubscription["criteria"];
}) {
    return <div className="space-y-1">
        <Badge variant={criteria ? "secondary" : "outline"}>{criteria ? "Conditional" : "All payloads"}</Badge>
        <span className="block max-w-full font-mono text-sm [overflow-wrap:anywhere]">{eventType || "All events"}</span>
    </div>;
}
...
function SubscriptionDesktopRow({
	...
	return (
		<DataTableRow clickable onClick={() => onOpenEdit(subscription)}>
			...
			<DataTableCell>
				<SubscriptionEventType criteria={subscription.criteria}
 eventType={subscription.event_type} />
			</DataTableCell>
			<DataTableCell className="text-right">
				{subscription.delivery_count || 0}
                {((subscription.skipped_count ?? 0) > 0 || (subscription.evaluation_error_count ?? 0) > 0) && <span className="block text-xs text-muted-foreground">{subscription.skipped_count ?? 0} skipped · {subscription.evaluation_error_count ?? 0} errors</span>}
			</DataTableCell>
			<DataTableCell className="text-right">
				{successRate !== null ? (
