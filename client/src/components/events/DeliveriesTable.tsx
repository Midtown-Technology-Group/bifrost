...
function DeliveryRecord({
	...
	const link =
		isAgent && delivery.agent_id && delivery.agent_run_id
			...
			: delivery.execution_id
				...
				: null;
	const decision = delivery.rule_decision;
	const state = delivery.status === "skipped" ? {
        label: decision?.outcome === "evaluation_error" ? "Criteria error" : "Criteria did not match",
        Icon: decision?.outcome === "evaluation_error" ? XCircle : AlertTriangle,
        color: decision?.outcome === "evaluation_error" ? "text-[var(--bf-danger)]" : "text-[var(--bf-warning)]",
    } : deliveryStates[
	...
	return (
		...
		>
			...
			{decision && <p className="text-sm text-muted-foreground">Rule: {String(decision.outcome)} · {String(decision.code)}</p>}
