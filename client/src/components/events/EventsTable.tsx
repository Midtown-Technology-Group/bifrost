...
function EventDeliveryCounts({ event }: { event: Event }) {
	return (
		<span className="flex flex-wrap gap-x-3 gap-y-1 text-sm">
            ...
            {(event.skipped_count ?? 0) > 0 && <span className="text-[var(--bf-warning)]">{event.skipped_count} skipped</span>}
            {(event.evaluation_error_count ?? 0) > 0 && <span className="text-[var(--bf-danger)]">{event.evaluation_error_count} criteria errors</span>}
